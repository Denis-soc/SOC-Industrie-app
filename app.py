...
with tab0:
    ...
    # Affichage du tableau (colonnes à adapter selon vos noms exacts dans Supabase)
    st.dataframe(df_stock[['num_interne', 'Nom du Matériel', 'taille', 'quantité']])

    # 2. Gestion du panier
    if "panier_stock" not in st.session_state:
    ...
    if st.session_state.panier_stock:
        ...
        if st.button("✅ Valider tout le panier"):
            for item in st.session_state.panier_stock:
                ...
                # Recherche de la ligne dans le DataFrame actuel
                mask = (df_stock['num_interne'] == ref)
                ligne = df_stock[mask]
                
                if not ligne.empty:
                    # Calcul du nouveau stock
                    stock_actuel = int(ligne.iloc[0]['quantité'])
                    if mvt_type == "Entrée":
                        new_stock = stock_actuel + qte
                    else:
                        new_stock = max(0, stock_actuel - qte)
                    
                    # Mise à jour Supabase uniquement sur num_interne
                    supabase.table("materiel").update({"quantité": new_stock}).eq("num_interne", ref).execute()
